...
%build
zig build            --global-cache-dir "%{_builddir}"/%{name}-%{version}-zig-cache -Dinit_system=systemd -Dcpu=baseline -Doptimize=ReleaseSmall -Dprefix_directory="/usr"

%install
zig build installexe --global-cache-dir "%{_builddir}"/%{name}-%{version}-zig-cache -Dinit_system=systemd -Dcpu=baseline -Doptimize=ReleaseSmall -Dprefix_directory="/usr" -Ddest_directory="%{buildroot}"
